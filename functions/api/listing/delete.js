...
export async function onRequestPost({ request, env }) {
  try {
    ...
    try {
      ...
      // If hard delete fails, fall back to marking deleted so it leaves public view.
      await env.DB.prepare(
        "UPDATE listings SET status = 'deleted', image_keys_json = '[]', image_sizes_json = '[]' WHERE listing_id = ? AND seller_id = ?"
      )
        .bind(listing.listing_id, seller.seller_id)
        .run();
    }
