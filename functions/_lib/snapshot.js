...
export async function buildPublicSnapshot(env) {
  ...
  const result = await env.DB.prepare(
    "SELECT listing_id, created_at, expires_at, rank, price_sek, brand, type, condition, wheel_size_in, features_json, faults_json, location, contact_mode, public_email, public_phone, image_keys_json FROM listings WHERE status = 'active' AND expires_at >= ?"
  )
    .bind(now)
    .all();
